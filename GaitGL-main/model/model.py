...
class Model:
    def __init__(self,
        ...
        self.m_resnet = nn.DataParallel(self.m_resnet, device_ids=[0])
        print(self.m_resnet)
    ...
    def transform(self, flag, batch_size=1):
        with torch.no_grad():
            ...
            data_loader = tordata.DataLoader(
                ...
                # pin_memory=True,
                # num_workers=self.num_workers
            data_loader1 = tordata.DataLoader(
                dataset=source1,
                # batch_size=batch_size,
                # sampler=tordata.sampler.SequentialSampler(source),
            ...
            for i, x in enumerate(data_loader):
                ...
                # if counttt%1000==0:
                #     print(label, view,'--',outputs.shape)
                counttt +=1
                n,_,_ = outputs.size()
                # n,_ = outputs.size()
            ...
            te1 = list()
            counttt1 = 0
            for i, x in enumerate(data_loader1):
            ...
            # print(outputs.view(n, -1).shape)
                outputs = outputs.view(n, -1).data.cpu().numpy()
                feature_list1.append(outputs)
                label_list1 += label
        # print(torch.equal(te[0],te[0]))

            print(1, feature_list)
            print(2, view_list)
            print(3, label_list)
            print(1, feature_list1)
            print(2, label_list1)
        return np.concatenate(feature_list, 0), view_list, label_list, np.concatenate(feature_list1, 0), label_list1
