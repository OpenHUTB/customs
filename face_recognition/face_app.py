...
class FaceRecognition(object):
    ...
    def __init__(self, encoding=0, **kwargs):
        ...
        self.det_model, self.rec_model = self._load_model()

        try:
            self.known_face_embeddings, self.known_face_names = self._load_faces()
        except:
            if not encoding:
    ...
    def add_face(self, image_path):
        ...
        if name not in name_list:
            ...
            self.known_face_embeddings = np.concatenate((self.known_face_embeddings, [embedding]), axis=0)
    ...
    def face_rec(self, image):
        ...
        if (bboxes.shape[0] != 0):
            ...
            for i in range(bboxes.shape[0]):
                ...
                embedding, face = self.rec_model.get(image, kps)
                matches, face_distances = compare_faces(self.known_face_embeddings, embedding)
                faces.append(face)
